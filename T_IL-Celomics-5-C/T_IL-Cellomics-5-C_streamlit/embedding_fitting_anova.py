...
for f in sorted(glob.glob(os.path.join(_base_dir, "embedding_fitting_Merged_Clusters_PCA_k*.csv"))):
    ...
    try:
        k_val = int(k_part)
        out_path = os.path.join(_base_dir, f"embedding_fitting_ANOVA - OneWay_k{k_val}.xlsx")
...
if not files_to_process:
    ...
    if os.path.exists(csv_path):
        files_to_process.append((csv_path, os.path.join(_base_dir, "embedding_fitting_ANOVA - OneWay.xlsx"), None))
