...
print("unique tracks:", df["unique_id"].nunique())
sizes = df.groupby("unique_id").size()
print("tracks with <48 points:", int((sizes < 48).sum()))
print("tracks with exactly 48:", int((sizes == 48).sum()))
print("tracks with >48:", int((sizes > 48).sum()))

# how many rows still have any NaN across features?
rows_any_nan = df[keep_feature_cols].isna().any(axis=1)
print("rows with ANY NaN across keep_feature_cols:", int(rows_any_nan.sum()))

# how many tracks have at least one NaN somewhere?
track_any_nan = df.assign(_any_nan=rows_any_nan).groupby("unique_id")["_any_nan"].any()
print("tracks with ANY NaN somewhere:", int(track_any_nan.sum()))
