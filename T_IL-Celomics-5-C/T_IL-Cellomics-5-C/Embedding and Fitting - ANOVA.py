...
for k, cluster_file in zip(k_values, cluster_files):
    ...
    os.makedirs(f"fitting/ANOVA{k_suffix}", exist_ok=True)
    ...
    # === 3. Define features ===
    exclude_cols = ['Experiment', 'Parent', 'Cluster', 'PC1', 'PC2', 'Treatment', 'dt', 'TimeIndex', 'ID', 'DoseLabel']
    features = [col for col in df_unique.columns if col not in exclude_cols and pd.api.types.is_numeric_dtype(df_unique[col])]
    ...
    # === 8. Save to Excel ===
    output_file = f'fitting/ANOVA{k_suffix}/embedding_fitting_ANOVA - OneWay{k_suffix}.xlsx'
