...
for k, cluster_file in zip(k_values, cluster_files):
    ...
    summary_df.to_excel(f"clustering/Descriptive_Table_By_Cluster_UniqueCells{k_suffix}.xlsx", index=False)
    print(f"Saved: clustering/Descriptive_Table_By_Cluster_UniqueCells{k_suffix}.xlsx")

    # === 8. Dose-stratified descriptive tables (if dose data available) ===
    if dose_data is not None and "DoseLabel" in df.columns:
        ...
        for dose_label in unique_doses:
            ...
            summary_dose_df.to_excel(f"clustering/Descriptive_Table_By_Cluster_Dose_{safe_dose_label}{k_suffix}.xlsx", index=False)
            print(f"Saved: Descriptive_Table_By_Cluster_Dose_{safe_dose_label}{k_suffix}.xlsx")
